...
// Generate UPI URL
const generateUPIUrl = (amount: number, orderId: string, name: string) => {
...
export default function PaymentPage({}: PaymentProps) {
...
Items:
${items.map(item => `${item.product.name} x${item.quantity} - ₹${(item.product.price || 0) * item.quantity}`).join('\n')}

Total Amount: ₹${total}

Payment Status: Processing`;

            // Get WhatsApp number and clean it
            ...
            // Create WhatsApp URL with simplified format
